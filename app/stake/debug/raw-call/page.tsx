  ...
  const makeRawCall = async () => {
    ...
    try {
      ...
      if (customData) {
        ...
        // Special handling for stakingInfo which requires two address parameters
        if (selectedFunction === "stakingInfo(address,address)") {
          // For stakingInfo, use the staking contract address as the second parameter
          const addressParam1 = targetAddress.toLowerCase().substring(2).padStart(64, "0")
          const addressParam2 = STAKING_CONTRACT_ADDRESS.toLowerCase().substring(2).padStart(64, "0")
          callData = `0x${FUNCTION_SIGNATURES[selectedFunction]}${addressParam1}${addressParam2}`
        } else {
          // Remove the double 0x prefix
